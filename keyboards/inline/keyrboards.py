...
async def generate_episode_keyboard(serial_id: int) -> InlineKeyboardMarkup:
    ...
    if not episodes:
        keyboard.add(InlineKeyboardButton(text="🎬 Hozircha epizodlar mavjud emas", callback_data="no_episodes"))
        return keyboard
    ...
    for season in sorted(grouped.keys()):
        if len(grouped) > 1:
            keyboard.add(InlineKeyboardButton(text=f"📁 {season}-FASL", callback_data="skip"))
...
admin_menu = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [
        InlineKeyboardButton(text="📽️ Seriallar bo'limi", callback_data="serials_movies")
    ],
    [
...
serials_movies_keyboard = InlineKeyboardMarkup(inline_keyboard=[
    [
        InlineKeyboardButton(text="📽️ Serial yaratish", callback_data="create_serial"),
    ],
    [
        InlineKeyboardButton(text="🗑️ Serialni o'chirish", callback_data="remove_serial"),
    ],
    [
        InlineKeyboardButton(text="✏️ Serialga qism qo'shish", callback_data="add_episode")
    ],
    [
        InlineKeyboardButton(text="🗑️ Serialdan qism O'chirish", callback_data="remove_episode")
    ],
    [
        InlineKeyboardButton(text="❌ Bekor qilish", callback_data="cancel_admin")
    ]
