...
def premium_status_buttons():
    ...
    status_text = (
        "Bot Premium So'rash ( Yoqilgan ) ✅"
        if current_status
        else "Bot Premium So'rash ( O'chirilgan ) ❌"
    )
        ...
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=status_text, callback_data="toggle_premium_status"
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="💰 Obuna narxini o‘zgartirish",
                    callback_data="change_money_price",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="💳 Carta raqam o'zgartirish",
                    callback_data="change_card_number",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="💳 Carta ega ismini o'zgartirish",
                    callback_data="change_card_owner",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="✴️ Obuna sotib olish help videosini sozlash",
...
async def ask_card_number(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "💳 Yangi karta raqamini kiriting:", reply_markup=cancel_admin
    )
...
async def save_card_number(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ Karta raqami yangilandi: <code>{msg.text}</code>",
        parse_mode="HTML",
...
async def ask_card_owner(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "👤 Yangi karta egasi ismini kiriting:", reply_markup=cancel_admin
    )
...
async def save_card_owner(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ Karta egasi ismi yangilandi: <b>{full_name}</b>",
        parse_mode="HTML",
        reply_markup=admin_menu,
    )
    await state.finish()
...
async def ask_new_price(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "💰 Yangi obuna narxini so‘mda kiriting:", reply_markup=cancel_admin
    )
...
async def save_new_price(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ Yangi obuna narxi belgilandi: {new_price} so‘m", reply_markup=cancel_admin
    )
    await state.finish()
