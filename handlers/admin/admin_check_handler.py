...
async def accept_payment(callback: types.CallbackQuery):
    ...
    await callback.message.edit_caption(
        callback.message.caption + f"\n\n✅ Qabul qilindi: {amount} so‘m",
        reply_markup=None,
    )
    await callback.bot.send_message(
        user_id, f"✅ To‘lovingiz tasdiqlandi. Hisobingizga {amount} so‘m qo‘shildi."
    )
...
async def reject_payment(callback: types.CallbackQuery):
    ...
    await callback.bot.send_message(
        user_id, "❌ To‘lovingiz rad etildi. Agar xatolik bo‘lsa, qayta urinib ko‘ring."
    )
    await callback.answer()
...
async def receive_custom_amount(msg: types.Message, state: FSMContext):
    ...
    await msg.bot.send_message(
        user_id, f"💰 Admin tomonidan hisobingizga {amount} so‘m qo‘shildi."
    )
