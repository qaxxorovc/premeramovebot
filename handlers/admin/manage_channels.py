...
from database.manage_tables import add_middleware_channel, remove_middleware_channel, get_all_channels
...
@dp.message_handler(content_types=types.ContentType.ANY, state=AddChannel.waiting_for_channel_forward)
async def handle_forwarded_channel(message: types.Message, state: FSMContext):
    if not message.forward_from_chat or message.forward_from_chat.type != "channel":
        await message.answer("❌ Iltimos, faqatgina OMMAVIY kanaldan to‘g‘ridan-to‘g‘ri xabarni yuboring (forward).")
        return
    ...
    try:
        ...
        await message.answer("🚫 Bot ushbu kanalga administrator sifatida qo‘shilmagan. Iltimos, botni kanalga admin qilib qo‘shing va qayta urinib ko‘ring.")
        await state.finish()
        return
    ...
    await message.answer(f"✅ <b>{channel_title}</b> kanali muvaffaqiyatli qo‘shildi!", parse_mode="HTML", reply_markup=admin_menu)
    await state.finish()
...
async def remove_channel_handler(call: types.CallbackQuery):
    ...
    for name, username, channel_id in channels:
        ...
        kb.add(
            InlineKeyboardButton(f"❌ O‘chirish — {name}", callback_data=f"delete_channel:{channel_id}")
        )
...
async def delete_channel_handler(call: types.CallbackQuery):
    ...
    if result == True:
        await call.message.answer("✅ Kanal muvaffaqiyatli o‘chirildi.", reply_markup=admin_menu)
        await call.message.delete()
    else:
