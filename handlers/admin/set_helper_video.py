...
def helper_video_buttons():
    ...
    if current_video:
        buttons.append([types.InlineKeyboardButton(text="🗑 Videoni o‘chirish", callback_data="delete_helper_video")])
    buttons.append([types.InlineKeyboardButton(text="📥 Yangi video yuklash", callback_data="set_helper_video")])
    buttons.append([types.InlineKeyboardButton(text="◀️ Ortga", callback_data="cancel_admin")])
...
async def show_helper_video_options(call: types.CallbackQuery):
    await call.message.edit_text(
        text="🎬 Quyidan helper videoni sozlang:",
        reply_markup=helper_video_buttons()
    )
    await call.answer()
...
async def ask_for_new_video(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("📥 Iltimos, yangi helper videoni tashlang:", reply_markup=cancel_admin)
    await SetHelperVideoState.waiting_for_video.set()
    await call.answer()

@dp.message_handler(content_types=types.ContentType.VIDEO, state=SetHelperVideoState.waiting_for_video)
...
async def delete_helper_video(call: types.CallbackQuery):
    ...
    await call.message.edit_text("🗑 Helper video o‘chirildi.", reply_markup=helper_video_buttons())
