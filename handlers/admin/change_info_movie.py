        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    "✏️ Nomi", callback_data=f"edit_field:name:{movie_id}"
                ),
                InlineKeyboardButton(
                    "🎥 Video", callback_data=f"edit_field:video:{movie_id}"
                ),
            ],
...
async def ask_movie_id(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "✏️ Qaysi filmni tahrirlashni xohlaysiz? Film ID raqamini yuboring:"
    )
...
async def process_movie_id(msg: types.Message, state: FSMContext):
    ...
    if not movie:
        await msg.answer(
            "<b>❌ Bunday ID bilan film topilmadi</b>\nTog'ri id kiriting.",
...
@dp.callback_query_handler(
    lambda c: c.data.startswith("edit_field:"), state=EditMovieState.choosing_field
)
...
@dp.message_handler(
    state=EditMovieState.entering_new_value, content_types=types.ContentType.ANY
)
async def update_field(msg: types.Message, state: FSMContext):
    ...
    if field == "video" and msg.video:
        ...
        await msg.reply(
            "❗️ Noto‘g‘ri format. To‘g‘ri yuboring.", reply_markup=cancel_admin
        )
        return
    ...
    await msg.answer(
        f"✅ Filmning {field} muvaffaqiyatli yangilandi.", reply_markup=admin_menu
    )
    await state.finish()
...
async def cancel_edit(callback: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await callback.message.answer(
        "❌ Tahrirlash bekor qilindi.", reply_markup=admin_menu
    )
    await callback.answer()
