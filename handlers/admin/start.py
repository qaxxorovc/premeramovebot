...
async def handle_start(message: types.Message, state: FSMContext):
    ...
    if args:
        ...
        await message.answer(f"✏️ Yangi qo‘shiladigan summani kiriting:", reply_markup=cancel_admin)
...
async def receive_amount_from_admin(msg: types.Message, state: FSMContext):
    ...
    await msg.bot.send_message(target_user_id, f"💰 Admin tomonidan hisobingizga {amount} so‘m qo‘shildi.")
    await msg.answer(f"✅ <code>{target_user_id}</code> foydalanuvchiga {amount} so‘m qo‘shildi.")
    await state.finish()
...
async def cancel_admin_funktion(event: types.Message | types.CallbackQuery, state: FSMContext):
    await state.finish()
