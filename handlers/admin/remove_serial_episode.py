...
async def confirm_serial_deletion(callback_query: types.CallbackQuery):
    ...
    if not row:
        await callback_query.message.answer(
            "❌ Serial topilmadi yoki allaqachon o‘chirilgan.", reply_markup=admin_menu
        )
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Ha, o‘chirilsin",
                    callback_data=f"confirm_delete_{serial_id}",
                ),
                InlineKeyboardButton(
                    text="❌ Yo‘q, bekor qilish", callback_data="cancel_admin"
                ),
            ]
        ]
    )
...
async def delete_serial(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        conn.commit()
        await callback_query.message.answer(
            "✅ Serial muvaffaqiyatli o‘chirildi.", reply_markup=admin_menu
        )
...
@dp.callback_query_handler(
    lambda c: c.data.startswith("removeep_serial:"),
    state=RemoveEpisode.waiting_for_serial_id,
)
async def ask_episode_for_removal(call: types.CallbackQuery, state: FSMContext):
    ...
    if not episodes:
        await call.message.edit_text(
            "❌ Bu serialda qismlar topilmadi.", reply_markup=admin_menu
        )
        return
...
@dp.callback_query_handler(
    lambda c: c.data.startswith("removeep_episode:"),
    state=RemoveEpisode.waiting_for_episode_id,
)
async def remove_episode_confirm(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "✅ Qism muvaffaqiyatli o‘chirildi.", reply_markup=admin_menu
    )
    await state.finish()
