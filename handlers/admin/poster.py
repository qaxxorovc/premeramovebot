...
@dp.message_handler(
    content_types=types.ContentType.ANY, state=AdminPost.waiting_for_post
)
async def receive_post(message: types.Message, state: FSMContext):
    ...
    kb.add(
        types.InlineKeyboardButton(
            "✏️ Sarlavhani tahrirlash", callback_data="edit_caption"
        ),
        types.InlineKeyboardButton(
            "📝 Postni qayta yozish", callback_data="rewrite_post"
        ),
    ...
    await message.answer(
        "Post foydalanuvchilarga quyidagicha ko‘rinadi 👇", reply_markup=kb
    )
    await AdminPost.waiting_for_action.set()


@dp.callback_query_handler(
    lambda c: c.data == "edit_caption", state=AdminPost.waiting_for_action
)
...
async def save_new_caption(message: types.Message, state: FSMContext):
    ...
    if old_msg.photo:
        await old_msg.bot.send_photo(
            chat_id=message.chat.id,
            photo=old_msg.photo[-1].file_id,
            caption=message.text,
        )
    elif old_msg.video:
        await old_msg.bot.send_video(
            chat_id=message.chat.id, video=old_msg.video.file_id, caption=message.text
        )
    elif old_msg.document:
        await old_msg.bot.send_document(
            chat_id=message.chat.id,
            document=old_msg.document.file_id,
            caption=message.text,
        )
    else:
        await message.answer(
            "❌ Bu media turida sarlavha qo‘shib bo‘lmaydi.", reply_markup=kb
        )
        return
...
@dp.callback_query_handler(
    lambda c: c.data == "rewrite_post", state=AdminPost.waiting_for_action
)
...
@dp.message_handler(
    content_types=types.ContentType.ANY, state=AdminPost.waiting_for_rewrite
)
...
@dp.callback_query_handler(
    lambda c: c.data == "cancel_post", state=AdminPost.waiting_for_action
)
...
@dp.callback_query_handler(
    lambda c: c.data == "send_to_all", state=AdminPost.waiting_for_action
)
async def send_to_all(call: types.CallbackQuery, state: FSMContext):
    ...
    for user_id in users:
        try:
            if msg.text:
                ...
                await msg.bot.send_video_note(
                    chat_id=user_id, video_note=msg.video_note.file_id
                )
            else:
                continue
            success += 1
    ...
    await call.message.answer(
        f"✅ {success} foydalanuvchiga yuborildi.\n❌ {failed} foydalanuvchiga yuborilmadi.",
